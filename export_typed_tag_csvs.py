...
def export_person_tags_with_keywords(output_dir: Path) -> tuple[Path, int]:
    data = json.loads(INDEX.read_text(encoding="utf-8"))
    rows = []
    for tag, entry in data.items():
        if entry.get("type") != "person":
            continue
        rows.append({"Person Tag": tag, "Aliases / Keywords": "; ".join(entry.get("keywords", []))})

    output_dir.mkdir(parents=True, exist_ok=True)
    path = output_dir / "ctcrazies_person_tags_with_aliases_keywords_2026-08-14.csv"
    with path.open("w", newline="", encoding="utf-8-sig") as handle:
        writer = csv.DictWriter(handle, fieldnames=["Person Tag", "Aliases / Keywords"])
        writer.writeheader()
        writer.writerows(sorted(rows, key=lambda item: item["Person Tag"].casefold()))
    return path, len(rows)
